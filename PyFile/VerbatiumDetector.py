...
The program is for SR's verbatim detection check process. It aims to reduce verbatim manual checking time and effort. But it can check a single row every time.
    ...
    The VerbatimCheck program does three things.

    Read three text files: rewrite.txt, model.txt, and reference txt
    All the text information comes from the spreadsheet manually.
    Extract the text data as a String, then store each sentence in the list object.

    Compare SR's rewrite and model response.
    ...
    ii. Rewrite the sentence which has the consecutive letters,
    iii. Model response sentence that has consecutive letters.

    Compare the raters' rewrite and references.
